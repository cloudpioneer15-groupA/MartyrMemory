...
import com.simpletech.MartyrMemory.annotations.dbmodel.interpreter.Interpreter;
...
public class BaseServiceImpl<T> implements BaseService<T>{
	...
	public List<T> findWhereByPage(String where, int limit, int start) throws Exception {
		return baseDao.findWhereByPage(where, limit, start);
	}
	...
	@Override
	public Page<T> listWhereByPage(String where, int pageSize, int pageNo) throws Exception {
		int limit = pageSize;
		int start = pageNo*pageSize;
		int totalRecord = baseDao.countAll();
		int totalPage = 1+totalRecord/pageSize;

		List<T> list = baseDao.findWhereByPage(where, limit, start);
